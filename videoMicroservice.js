    ...
    getvideo: (call, callback) => {
        ...
        pool.getConnection((err, connection) => {
          ...
          connection.query(query, [videoId], (error, results) => {
            ...
            if (results.length === 0) {
              const errorMessage = `Video with ID ${videoId} not found`;
      ...
      searchvideos: (call, callback) => {
        const { query } = call.request;
        const searchQuery = '%' + query + '%';
        const searchVideosQuery = 'SELECT * FROM videos WHERE title LIKE ? OR description LIKE ?';
